...
namespace Accord.AMath
{
    ...
    public static partial class Matrix
    {
        ...
        {
            int[] indices = Accord.AMath.Vector.Range(keys.Length);
            ...
            where T : IComparable<T>
        {
            ...
            if (count > values.Length)
                return Accord.AMath.Vector.Range(values.Length);

            T[] work = (inPlace) ? values : (T[])values.Clone();
            int[] idx = Accord.AMath.Vector.Range(values.Length);
            work.NthElement(idx, 0, work.Length, count, asc: false);
            StreamerBotLib.MachineLearning.Accord.Sort.Insertion(work, idx, 0, count, asc: false);
        ...
        public static int Partition<T, TValue>(this T[] list, TValue[] keys, int left, int right, bool asc = true)
            where T : IComparable<T>
        {
            return StreamerBotLib.MachineLearning.Accord.Sort.Partition(list, left, right, asc);
        }
        ...
        {
            return StreamerBotLib.MachineLearning.Accord.Sort.Partition(list, keys, left, right, compare, asc: asc);
        }
        ...
        {
            return StreamerBotLib.MachineLearning.Accord.Sort.Partition(list, left, right, compare, asc: asc);
        }
